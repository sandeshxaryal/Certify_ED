...
/**
 * Builds the exact string that gets digitally signed by the issuing
 * institute's RSA private key, and that must be reconstructed identically
 * at verification time using the same stored fields.
 */
export const buildSigningPayload = (certificateId, referenceId, candidateName, institutionName, issuedDate, issuerId) => {
  return `${certificateId}|${referenceId}|${candidateName}|${institutionName}|${issuedDate}|${issuerId}`;
};
